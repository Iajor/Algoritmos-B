...
void listagem(Participante vetor[], int totalInscritos) {
    ...
    for (int i = 0; i < totalInscritos-1; i++) {
        cout << vetor[i].matricula << ":" << vetor[i].nome << endl;
    }
}


void registrarEntrada(Participante vetor[], int totalInscritos) {
    fflush(stdin);
    string matricula;
    ...
    for (int i = 0; i < totalInscritos; i++) {
        if (vetor[i].matricula == matricula) {
            ...
            char hora[9];
            strftime(hora, sizeof(hora), "%H:%M:%S", tempoLocal);
            ofstream procuradorArquivo;
            procuradorArquivo.open("entradas.csv", ios::out | ios::app);
            procuradorArquivo << vetor[i].matricula << ";" << hora << endl;
            procuradorArquivo.close();
            cout << "Entrada registrada para " << vetor[i].nome << " as " << hora << endl;
            break;
        }
    }

    if (!encontrado) {
        cout << "Matricula nao encontrada nos inscritos." << endl;
    }
}

void registrarSaida(Participante vetor[], int totalInscritos) {
    fflush(stdin);
    string matricula;
    cout << "Informe a matricula: ";
    cin >> matricula;
    ...
    for (int i = 0; i < totalInscritos; i++) {
        if (vetor[i].matricula == matricula) {
            ...
            strftime(hora, sizeof(hora), "%H:%M:%S", tempoLocal);
            ofstream procuradorArquivo;
            procuradorArquivo.open("saidas.csv", ios::out | ios::app);
            procuradorArquivo << vetor[i].matricula << ";" << hora << endl;
            procuradorArquivo.close();
            cout << "Saida registrada para " << vetor[i].nome << " as " << hora << endl;
            break;
        }
    }
...
void menu(Participante vetor[], int n, int totalInscritos) {  
    ...
    do {
        ...
        case 3:
            ...
            registrarEntrada(vetor, totalInscritos);
            break;
        case 4:
            cout << "Registrar saida\n";
            registrarSaida(vetor, totalInscritos);
            break;
        case 5:  
            cout << "Encerrando...";         
            exit(1);
